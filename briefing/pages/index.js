...
function Tag({ t, pill }) {
  ...
  return (
    <span style={{
      fontSize: 9.5, fontWeight: 500, letterSpacing: "0.8px",
      textTransform: "uppercase", background: s.bg, color: s.color,
      padding: pill ? "4px 10px" : "3px 9px",
...
const LOAD_MSGS = [
  ...
  "A compilar destaques de tecnologia…",
...
export default function Home() {
  ...
  return (
    <>
      ...
      <style>{`
        :root {
          --bg:#f7f6f2; --bg-card:#fff; --bg-nav:rgba(247,246,242,.95);
        ...
        @media(prefers-color-scheme:dark){
          :root{
            --bg:#141412; --bg-card:#1e1d1a; --bg-nav:rgba(20,20,18,.95);
        ...
        *{box-sizing:border-box;margin:0;padding:0}
        ...
        button.tab:hover:not(.active){color:var(--ink-2)!important}
      `}</style>
      ...
      <nav style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"1rem 2rem",borderBottom:"0.5px solid var(--border)",background:"var(--bg-nav)",position:"sticky",top:0,zIndex:100,backdropFilter:"blur(16px)"}}>
        ...
        <button
          ...
          style={{background:"none",border:"0.5px solid var(--border-md)",borderRadius:20,padding:"5px 14px",fontSize:11,fontFamily:"'DM Sans',sans-serif",color:"var(--ink-2)",cursor:phase==="loading"?"not-allowed":"pointer",opacity:phase==="loading"?.5:1,display:"flex",alignItems:"center",gap:5,transition:"all .2s"}}
        >
          <span style={{animation:phase==="loading"?"spin 1s linear infinite":"none",display:"inline-block"}}>↻</span>
          Atualizar
        </button>
      </nav>
      ...
      <div style={{display:"flex",padding:"0 2rem",background:"var(--bg-card)",borderBottom:"0.5px solid var(--border)",overflowX:"auto"}}>
        {TABS.map(t => (
          <button key={t} className={`tab${filter===t?" active":""}`}
            onClick={() => setFilter(t)}
            style={{fontSize:11.5,color:filter===t?"var(--ink)":"var(--ink-3)",padding:"10px 14px",border:"none",background:"none",cursor:"pointer",borderBottom:`1.5px solid ${filter===t?"var(--accent)":"transparent"}`,fontFamily:"'DM Sans',sans-serif",whiteSpace:"nowrap",transition:"all .15s"}}
          >{TAB_LABELS[t]}</button>
        ))}
      </div>
      ...
      <div style={{background:"var(--bg-card)",padding:"3rem 2rem 2.5rem",borderBottom:"0.5px solid var(--border)",position:"relative",overflow:"hidden",minHeight:180}}>
        ...
        {phase==="done" && hero && (
          <>
            <div style={{fontSize:10,fontWeight:500,letterSpacing:2,textTransform:"uppercase",color:"var(--accent)",marginBottom:"1rem"}}>
              Briefing · {dateStr}
            </div>
            <h1 style={{fontFamily:"'Playfair Display',serif",fontSize:"clamp(22px,3.5vw,36px)",lineHeight:1.22,color:"var(--ink)",maxWidth:780,marginBottom:"1.1rem"}}>
              {hero.headline}
            </h1>
            <p style={{fontSize:15,fontWeight:300,lineHeight:1.72,color:"var(--ink-2)",maxWidth:680}}>
              {hero.summary}
            </p>
      ...
      {phase==="done" && genAt && (
        <div style={{padding:"0.55rem 2rem",fontSize:11,color:"var(--ink-3)",borderBottom:"0.5px solid var(--border)",display:"flex",alignItems:"center",gap:6}}>
          <span style={{width:5,height:5,borderRadius:"50%",background:"#4caf50",flexShrink:0,display:"inline-block"}} />
          Atualizado às {new Date(genAt).toLocaleTimeString("pt-PT",{hour:"2-digit",minute:"2-digit"})}
          {" "}· {cards.length} notícias · NewsAPI + Groq AI (gratuito)
        </div>
      )}
      ...
      <footer style={{padding:"2rem",textAlign:"center",fontSize:10.5,color:"var(--ink-3)",borderTop:"0.5px solid var(--border)",marginTop:"1rem",letterSpacing:"0.5px",lineHeight:1.8}}>
        Briefing Diário · NewsAPI · Groq AI · ZeroHedge · The Economist · ARK Invest · DWS · Guggenheim · BofA<br/>
        100% gratuito · Pesquisa em tempo real · Atualiza a cada carregamento
      </footer>
    </>
  );
}
